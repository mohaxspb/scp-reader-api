ALTER TABLE articles_langs
  drop constraint IF EXISTS articles_langs_pkey CASCADE;
ALTER TABLE articles_langs
...
ALTER TABLE articles_images
  drop constraint IF EXISTS articles_images_pkey CASCADE;
ALTER TABLE articles_images
  drop constraint IF EXISTS articles_images_article_id_fkey CASCADE;
ALTER TABLE articles_images
  ADD COLUMN IF NOT EXISTS id SERIAL PRIMARY KEY;
ALTER TABLE articles_images
  ADD COLUMN IF NOT EXISTS article_for_lang_id BIGINT;
ALTER TABLE articles_images
  drop if exists article_lang_id,
  drop if exists article_id,
  drop if exists article_url_relative;
ALTER TABLE articles_images
  drop constraint IF EXISTS fk_article_image_article_langs CASCADE;
alter table articles_images
