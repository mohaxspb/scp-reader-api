...
class CacheService @Autowired constructor(
    ...
    sealed class CacheStatus {
        object NotPopulated : CacheStatus()
        object Populating : CacheStatus()
        data class Populated(
            val categoriesArticlesCacheSize: Long,
            val articlesByIdCacheSize: Long,
            val articlesByUrlAndLangCacheSize: Long,
            val searchResultsSize: Long
        ) : CacheStatus()
    }

    val cacheStatus: AtomicReference<CacheStatus> = AtomicReference(CacheStatus.NotPopulated)

    @Async
    fun populateCache() {
        cacheStatus.set(CacheStatus.Populating)
        try {
            ...
            ScpReaderConstants.Firebase.FirebaseInstance.values().forEach { langEnum ->
                ...
                val recentCacheInitialSize =
                    serverSettingsService.findByKey(ServerSettings.Key.MOST_RECENT_ARTICLES_CACHE_SIZE.name)
                ...
                val ratedCacheInitialSize =
                    serverSettingsService.findByKey(ServerSettings.Key.MOST_RATED_ARTICLES_CACHE_SIZE.name)
            ...
            val caffeineArticlesByIdCache =
                (cacheManager.getCache(ARTICLE_TO_LANG_DTO_BY_ID) as CaffeineCache).nativeCache
            log.error("caffeineArticlesByIdCache estimatedSize: ${caffeineArticlesByIdCache.estimatedSize()}")

            val caffeineArticlesByLangAndUrlRelativeCache =
            ...
            log.error("caffeineSearchCache estimatedSize: ${caffeineSearchCache.estimatedSize()}")

            cacheStatus.set(
                CacheStatus.Populated(
                    categoriesArticlesCacheSize = caffeinCategoriesArticlesCache.estimatedSize(),
                    articlesByIdCacheSize = caffeineArticlesByIdCache.estimatedSize(),
                    articlesByUrlAndLangCacheSize = caffeineArticlesByLangAndUrlRelativeCache.estimatedSize(),
                    searchResultsSize = caffeineSearchCache.estimatedSize(),
                )
            )
        } catch (e: Exception) {
            val stackTrace = errorUtils.stackTraceAsString(e)
            log.error("Error while populating cache:\n$stackTrace")
            cacheStatus.set(CacheStatus.NotPopulated)
        }
    }
