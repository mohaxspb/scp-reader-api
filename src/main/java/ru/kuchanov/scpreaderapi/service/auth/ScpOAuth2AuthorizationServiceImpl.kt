...
@Service
class ScpOAuth2AuthorizationServiceImpl @Autowired constructor(
    /**
    ...
    private val tokenConverter: Converter<OAuth2AccessTokenResponse, Map<String, Any>>
) : ScpOAuth2AuthorizationService {
    ...
    override fun save(authorization: OAuth2Authorization) {
        ...
        oauthAuthorizationNewRepository.saveAndFlush(authToSave)
    }
...
//                    val accessToken = accessTokenService.findFirstByRefreshToken(token)
                    val accessToken = accessTokenService.findFirstByToken(token)
                        ?: return null
                    //fixme test return null instead of throwing NPE
...
//            val user = userDetailsService.loadUserByUsername(email)
            ...
            if (principalFromAuth is UserDetails) {
                principal = principalFromAuth.username
...
//                principal = null
            ...
            tokenNew = email?.let { oauthAuthorizationNewRepository.findFirstByPrincipalName(email) }
        ...
        return tokenConverter.convert(tokenResponse)!!
    }
}